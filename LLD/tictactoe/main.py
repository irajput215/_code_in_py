...
def main():
    ...
    player1 = Player("Ishu", Symbol.X)
    player2 = Player("Alex", Symbol.O)
    
    # 2. Strategy Selection: We choose which algorithm to use.
    # This makes the system flexible and easy to test.
    size = 3
    winning_strategy = OrderOneWinningStrategy(size)
    
    # 3. Initialize Game
    game = GameController(size, [player1, player2], winning_strategy)
